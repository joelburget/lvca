...
let string_location ~str ~loc =
  ...
  then
    ...
    div ~at:(classes "flex flex-row font-mono mx-2 bg-gray-100 underline")
...
let view_root_snapshot str root =
  ...
  let status_view = current_snapshot_s
    |> S.map ~eq:html_eq
      (fun Model.TraceSnapshot.{ success; _ } -> span
  ...
  table ~at:[class' "w-full"]
    [ tr [ td ~at:(classes "p-2 border-b-2 border-r-2") [txt "stack"]
    ...
    ; tr [ td ~at:(classes "p-2 border-r-2") [txt "subparsers"]
...
module View = struct
  ...
  let mk_input_result
    ...
    let parser_hl_s = tm_selection_s
      |> S.map ~eq:SourceRanges.(=) (SourceRanges.restrict ~buf:"parser")
    in
    ...
    let trace_cell = S.l2 ~eq:html_eq
    ...
    let mk_row left right = tr
      [ td ~at:(classes "border-2 p-2") left
      ; td ~at:(classes "border-2 p-2") right
      ]
    in
    div ~at:(classes "grid grid-cols-4")
      [ table ~at:(classes "font-mono mb-6 col-span-4 table-fixed")
        [ tr
  ...
  let view model =
    ...
    let pg_parser_input, set_pg_parser_input =
      S.create ~eq:String.(=) Examples.fix
    in
    ...
    div
      ...
      ; p [ txt "Let's start with the simplest class of parsers, which accept a single character or a fixed string." ]
      ...
      ; h4 [ code_inline' "'c'" ]
      ; p [ txt "A single-quoted character accepts exactly that character. Note that this example, like many of the others is (intentionally) failing initially. Try changing the input so it's accepted." ]
      ; char_table
      ...
      ; p [ txt "For convenience, I'll leave the last two parsers in scope as "
          ; code_inline' "alpha"
          ; txt " and "
          ; code_inline' "digit"
      ...
      ; p [ txt "Concatenating a sequence of parsers accepts when they all parse successfully in sequence. A parser must return something, which goes to the right of the arrow. For example "
          ; code_inline' Examples.sequence1
          ; txt " parses a simple addition expression where the operands, <code>a</code> and <code>b</code>, are both one character (any character)."
          ]
      ; sequence1_table
      ...
      ; p
        ...
        ; txt ". This example as written is of course not very useful, but this can be quite useful as part of a larger parser."
        ]
      ; fail_table
