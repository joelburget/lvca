...
module View = struct
  ...
  (*
  let insert_demo = function
    | TermAndConcretePage -> TermAndConcrete.insert_demo
  ...
  let handler signal_update evt =
    ...
    let select_elem = elem
      |> Dom_html.CoerceTo.select
      |> Js.Opt.to_option
      |> Option.value_exn
    in
    let i = select_elem##.value
      |> Js.to_string
      |> Int.of_string
    in
    Caml.Printf.printf "%i\n" i;
    let page = match List.nth all_pages i with
    ...
    (*
    wrapper_dom##.innerHTML := Js.string "";
    Dom.appendChild wrapper_dom
      (page |> stateless_view |> Js_of_ocaml_tyxml.Tyxml_js.To_dom.of_div);
      *)
    false
      ...
      [ select
        ...
        ( all_pages
          |> List.mapi ~f:(fun i page ->
            option ~a:[a_value (Int.to_string i)] (page |> page_description |> txt)
          )
        )
      ; R.Html5.div
        (model_s
