...
let elem_of_env = fun { var_types; _ } ->
  let vars = var_types
    |> Map.to_alist
    |> List.map ~f:(fun (name, ty) ->
      li []
        [ text name
        ; text (string_of_term ty)
        ])
  in
...
let string_of_typing = fun (Typing (tm, ty)) ->
...
let elem_of_trace_entry = function
  | CheckTrace (env, typing) ->
    div []
      [ h3 [] [ text "check" ]
      ; elem_of_env env
      ; text (string_of_typing typing)
      ]
  | InferTrace (env, term) ->
    div []
      [ h3 [] [ text "infer" ]
      ; elem_of_env env
      ; text (string_of_term term)
      ]
  | CheckSuccess ->
    div []
      [ h3 [] [ text "check success" ]
      ]
  | CheckFailure(_) ->
    div []
      [ h3 [] [ text "check failure" ]
      ]
  | Inferred(ty) ->
    div []
      [ h3 [] [ text "inferred" ]
...
let elem_of_current_stack = fun current_stack ->
  let last_row, stack = match current_stack with
  ...
  let other_rows = stack
    |> List.map ~f:(fun trace_entry ->
      tr []
        [ td [] [ elem_of_trace_entry trace_entry ]
        ; td [] []
        ]
    )
    |> List.rev
...
module Term_render_component = struct
  ...
  let apply_action ~inject:_ ~schedule_event:_ _input model = function
    ...
    | StaticsUpdate statics_result
    -> { model with statics = Some statics_result }
    | (StepForward | StepBackward) as step
    -> let debugger = match model.debugger with
      | None -> None
      | Some state -> (match step with
        | StepForward
        -> Some { state with current_step = state.current_step + 1 }
        | _
        -> Some { state with current_step = state.current_step - 1 }
      )
      in { model with debugger }
    | ToggleAbstract
    -> { model with abstract_expanded = not model.abstract_expanded }
    | ToggleStatics
    -> { model with statics_expanded = not model.statics_expanded }
    | ToggleDebugger
    -> { model with debugger_expanded = not model.debugger_expanded }
    | Evaluate input
    -> (match model.statics with
         | Some (Ok rules) ->
           ...
           let debugger = match parse_cvt input with
             | exception _ -> None
             | tm -> (match infer_trace handle_trace env tm with
               | exception _ -> None
               | _ty -> Some
                 Model.{ input; steps = Queue.to_list steps; current_step = 0 }
             )
           in
           { model with debugger }
  ...
  let compute : inject:(Action.t -> Vdom.Event.t) -> Input.t -> Model.t -> Result.t =
   fun ~inject _input state ->
     ...
     let statics_valid = match state.statics with
       | Some (Ok _) -> true
       | _ -> false
     in

     let mk_section = fun name enabled expanded toggle_evt contents ->
       div [ classes ("section-box" :: (if enabled then [] else ["disabled"])) ]
         [ h3 []
           [ a [ href "#"; on_click (fun _evt -> inject toggle_evt) ]
     ...
     *)
     ...
     let debugger = mk_section "Debugger" (abstract_valid && statics_valid)
       state.debugger_expanded ToggleDebugger (
         let debugger_input =

           let handle_keyup = fun evt ->
             if Web_util.is_enter evt
             then (match Web_util.str_of_input_evt evt with
         ...
         let debugger_results = match state.debugger with
           | None -> none
           | Some { input; steps; current_step } ->
             ...
             div []
               ...
               ; div []
                 [ text (Printf.sprintf "%n / %n" (current_step + 1) step_count) ]
               ; back_button
               ; forward_button
               ; table []
                 [ thead []
...
let initial_model = Term_render_component.Model.
