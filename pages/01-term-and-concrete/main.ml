...
module Term_render_component = struct
  ...
  let apply_action ~inject:_ ~schedule_event:_ () _model action
    = match action with
    | Action.UpdateInput (side, str)
    -> Model.Produced (side, str, Error "...")
    | Evaluate (side, str)
    -> Produced (side, str, P_term.parse str)

  let compute
    : inject:(Action.t -> Event.t) -> Input.t -> Model.t -> Result.t
    = fun ~inject () model ->

    let handle_keydown side = Attr.on_keydown (fun evt ->
    ...
    let mk_textarea side input_str = Node.(textarea (Attr.(
