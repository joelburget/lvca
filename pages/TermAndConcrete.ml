...
module Model = struct
  ...
  let print { input; input_lang; result; input_selected; output_selected } =
    ...
    Fmt.pr
      "{ input = %s; input_lang = %s; result = %a; input_selected = %a; output_selected = %a }\n"
      input
      input_lang_str
      (fun ppf tm_result ->
      ...
      result
      OptRange.pp input_selected
      OptRange.pp output_selected
  ;;
...
module Controller = struct
  let update (action : Action.t) model =
    ...
    | SwitchInputLang ->
      ...
      in
      { input; input_lang = input_lang; input_selected = None; output_selected = None; result }
  ;;
end
...
module View = struct
  let mk_output' model_s =
    ...
    in
    let s = model_s
      |> S.map ~eq:(Tuple2.equal phys_equal html_eq) (fun Model.{ result; input_lang; _ } ->
        let elt, formatter, output_selection_e, _clear = RangeFormatter.mk ~selection_s in
  ...
  let view model_s =
    ...
    let evts = E.select
      ...
      ; output_selection_e |> E.map (fun rng -> match Map.find rng "input" with
