...
module View = struct
  ...
  let bind_event ev elem handler =
    ...
    (*
  let textarea = Html5.(textarea (txt "") ~a:[
    a_autofocus ();
    a_rows 25;
    a_cols 90;
    ])
  let textarea_dom = To_dom.of_textarea textarea
  let set_textarea v = textarea_dom##.value := Js.string v
  let focus_textarea () = textarea_dom##focus
  *)
  ...
  let key_handler rp : Codemirror.codemirror_type Js.t -> Dom_html.keyboardEvent Js.t -> bool
    = fun cm evt ->
    ...
    then
      ...
      set_katex str;
      Controller.update (Evaluate str) rp
  ...
  (*
  let textarea' (rp : react_pair) =
    bind_event Ev.keypresses textarea_dom (fun evt ->
      Lwt.return @@ (let (_ : bool) = key_handler rp evt in ()));
    textarea
    *)

  let view (_rp : react_pair) = Html5.div [
    (* textarea' rp; *)
    codemirror_elem;
    katex_area;
  ]
...
let main _ =
  ...
  View.codemirror##on_change (Js.string "change")
    (Js.wrap_callback (fun _ -> Printf.printf "change 2\n"));

  (*
  View.codemirror##on (Js.string "cursorActivity")
    (Js.wrap_callback (fun _ -> Printf.printf "cursor activity\n"));
  View.codemirror##on (Js.string "focus")
    (Js.wrap_callback (fun _ -> Printf.printf "focus\n"));
  View.codemirror##on (Js.string "update")
    (Js.wrap_callback (fun _ -> Printf.printf "update\n"));
    *)

  View.codemirror##on_keypress (Js.string "keypress")
    (Js.wrap_callback (fun cm evt -> let (_ : bool) = View.key_handler react_pair cm evt in ()));
  Lwt.return ()
