...
module Model = struct
  module Evaluation = struct
    ...
    let (=) ev1 ev2 = String.(ev1.input = ev2.input) &&
  ...
  let (=) m1 m2 = List.equal Evaluation.(=) m1.evaluations m2.evaluations &&
      ...
      | Evaluate input ->
        let model = match ParseUtil.parse_string Parse.t input with
          ...
          | Ok parsed ->
            ...
            Model.
              { evaluations = { input; parsed; digits } :: evaluations
              ; error_msg = None
              }
        in
        model
      | DeleteRow i ->
        { evaluations = Lvca_util.List.remove_nth evaluations i; error_msg }
...
module View = struct
  ...
  let view model_s =
    ...
    let examples =
      ul (examples |> List.map ~f:(fun example -> li [ example ]))
    in
    ...
    let row' row_num input_str parsed digits_s _digits_update =
      ...
      let digits_s = digits_s
        |> S.map ~eq:Common.html_eq (fun digits -> match Calculator.interpret parsed with
          ...
          | Ok real ->
            ...
            span [txt str])
      in
      let digits' = mk_reactive' pre
        ~eq:Common.htmls_eq ~at:(classes "whitespace-pre-wrap break-all")
        digits_s
      in
      ...
      let evts = E.select
        [ digits_event
          |> E.map (fun update -> Action.ChangePrecision (row_num, update))
        ; Evr.on_el Ev.click (fun _evt -> Action.DeleteRow row_num) delete_button
        ]
      in

      let elem = row
        [ td ~at:(classes "py-4 pr-1")
    ...
    let tbody, tbody_evts =
      ...
      let s = model_s
        |> S.map ~eq (fun model -> model.Model.evaluations
    ...
    let error_msg = model_s
      |> S.map ~eq:Common.htmls_eq (fun model -> match model.Model.error_msg with
        | None -> []
    ...
    let actions = E.select
      ...
      ; example_update_es
        |> E.select
        |> E.map (fun example -> Action.Evaluate example)
      ; tbody_evts
        |> S.map ~eq:phys_equal E.select (* Select one event from a list of events *)
        |> E.swap (* Extract current signal's event *)
      ]
    in
