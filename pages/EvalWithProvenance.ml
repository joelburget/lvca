...
module Model = struct
  ...
  let print { input; parsed_input; input_lang; result; input_selected; output_selected } =
    ...
    Fmt.pr
      "{ input = %s; parsed_input = %a; input_lang = %s; result = %a; input_selected = %a; output_selected = %a }\n"
      input
...
let stateless_view () =
  let initial_model : Model.t =
    ...
    let result = Result.bind parsed_input ~f:eval in
    { input; parsed_input; input_lang = Lambda; result; input_selected = None; output_selected = None }
  in
