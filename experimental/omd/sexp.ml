...
and inline = function
  | Concat xs -> List (Atom "concat" :: List.map inline xs)
  | Text s -> Atom s
  | Emph e -> List [ Atom "emph"; inline e.content ]
  | Code _ -> Atom "code"
  | Hard_break -> Atom "hard-break"
  | Soft_break -> Atom "soft-break"
  | Link { kind = Url; def; _ } -> List [ Atom "url"; link_def inline def ]
  | Ref { kind = Url; label; def; _ } ->
    List [ Atom "url-ref"; inline label; link_def atom def ]
  | Html s -> List [ Atom "html"; Atom s ]
  | Link { kind = Img; _ } -> Atom "img"
  | Ref { kind = Img; label; def; _ } ->
    List [ Atom "img-ref"; inline label; link_def atom def ]
  | Tag { tag; content; _ } -> List [ Atom "tag"; Atom tag; inline content ]
;;

let rec block = function
  | Paragraph x -> List [ Atom "paragraph"; inline x ]
  | List l ->
    List
      (Atom "list"
      :: List.map (fun xs -> List (Atom "list-item" :: List.map block xs)) l.blocks)
  ...
  | Code_block { kind = None; code = Some s; _ } -> List [ Atom "indented-code"; Atom s ]
  | Code_block { kind = None; code = None; _ } -> List [ Atom "indented-code" ]
  | Code_block { kind = Some _; code = Some s; _ } -> List [ Atom "fenced-code"; Atom s ]
  | Code_block { kind = Some _; code = None; _ } -> List [ Atom "fenced-code" ]
  | Html_block s -> List [ Atom "html"; Atom s ]
  ...
  | Def_list { content } ->
    List
      ...
      ; List
          (List.map
             (fun elt ->
               List [ inline elt.Def_list.term; List (List.map inline elt.defs) ])
             content)
      ]
