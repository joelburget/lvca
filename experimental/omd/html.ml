...
let concat p b l = List.iter (p.inline p b) l
let text _ b t = Buffer.add_string b (htmlentities t)
...
let hard_break _ b = Buffer.add_string b "<br />\n"
let soft_break _ b = Buffer.add_string b "\n"
let html _ b body = Buffer.add_string b body
...
let link p b l =
  ...
  | Url -> url p b l.def.label l.def.destination l.def.title l.def.attributes
  | Img -> img p b l.def.label l.def.destination l.def.title l.def.attributes
;;

let ref p b r =
  match r.Ref.kind with
  | Url -> url p b r.label r.def.destination r.def.title r.def.attributes
  | Img -> img p b r.label r.def.destination r.def.title r.def.attributes
;;

let tag p b t = p.inline p b t.Tag.content

let inline p b = function
  | Concat l -> p.concat p b l
  | Text t -> p.text p b t
  | Emph e -> p.emph p b e
  | Code c -> p.code p b c
  | Hard_break -> p.hard_break p b
  | Soft_break -> p.soft_break p b
  | Html body -> p.html p b body
  | Link l -> p.link p b l
  | Ref r -> p.ref p b r
  | Tag t -> p.tag p b t
;;
...
let list p b l =
  ...
  List.iter
    (fun x ->
      ...
      Buffer.add_string b "</li>\n")
    l.blocks;
  Buffer.add_string b (match l.kind with Ordered _ -> "</ol>" | Unordered _ -> "</ul>")
;;
...
let code_block p b c =
  ...
  | { label = Some language; code = None; _ } ->
    Buffer.add_string
      b
      (Printf.sprintf "><code class=\"language-%s\"></code></pre>" language)
...
let thematic_break _ b = Buffer.add_string b "<hr />"
let html_block _ b body = Buffer.add_string b body
...
let tag_block p b t =
  let f i block =
    p.block p b block;
    if i < List.length t.Tag_block.content - 1 then Buffer.add_char b '\n'
  in
  List.iteri f t.Tag_block.content
;;

let block p b = function
  | Blockquote q -> p.blockquote p b q
  | Paragraph md -> p.paragraph p b md
  | List l -> p.list p b l
  | Code_block c -> p.code_block p b c
  | Thematic_break -> p.thematic_break p b
  | Html_block body -> p.html_block p b body
  | Heading h -> p.heading p b h
  | Def_list l -> p.def_list p b l
  | Tag_block t -> p.tag_block p b t
  | Link_def _ -> ()
;;
