...
module P : sig
  ...
  let peek_exn st = if st.pos >= String.length st.str then raise Fail else st.str.[st.pos]
...
let sp3 s =
  ...
  | Some ' ' ->
    ...
    | Some ' ' ->
      let s = Sub.tail s in
      (match Sub.head s with Some ' ' -> 3, Sub.tail s | Some _ | None -> 2, s)
    | Some _ | None -> 1, s)
  | Some _ | None -> 0, s
;;
      ...
      let rec loop' s =
        ...
        | Some '}' ->
          ...
          | Some _ ->
            ...
            Buffer.add_char buf '}';
            loop s
          | None -> Sub.of_string (Buffer.contents buf), Some (Buffer.contents buf'))
        | None ->
          Buffer.add_char buf '{';
          Buffer.add_buffer buf buf';
...
let atx_heading s =
  let rec loop n s =
    ...
    | Some _ -> raise Fail
    | None -> Latx_heading (n, Sub.to_string s, Attributes.empty)
  in
  loop 0 s
;;
...
let entity s =
  ...
  | ('a' .. 'z' | 'A' .. 'Z') :: _ ->
    let rec loop len t =
      ...
      | Some ('a' .. 'z' | 'A' .. 'Z' | '0' .. '9') -> loop (succ len) (Sub.tail t)
      | Some ';' ->
        let name = Sub.to_string (Sub.sub ~len s) in
        (match Entities.f name with [] -> raise Fail | cps -> cps, Sub.tail t)
      | Some _ | None -> raise Fail
    in
    loop 1 (Sub.tail s)
  | _ -> raise Fail
...
let unordered_list_item ind s =
  ...
  | Some (('+' | '-' | '*') as c) ->
    let s = Sub.tail s in
    if is_empty s
    then Llist_item (Unordered c, 2 + ind, s)
    else (
      let n = indent s in
...
let tag_name s0 =
  ...
  | Some ('a' .. 'z' | 'A' .. 'Z') ->
    let rec loop len s =
      match Sub.head s with
      | Some ('a' .. 'z' | 'A' .. 'Z' | '0' .. '9' | '-') -> loop (succ len) (Sub.tail s)
...
let attribute_name s =
  ...
  | Some ('a' .. 'z' | 'A' .. 'Z' | '_' | ':') ->
    let rec loop s =
      match Sub.head s with
      | Some ('a' .. 'z' | 'A' .. 'Z' | '_' | '.' | ':' | '0' .. '9') -> loop (Sub.tail s)
      | Some _ | None -> s
    in
    loop s
  | Some _ | None -> raise Fail
...
let open_tag s =
  ...
  let s = ws s in
  let n =
    match Sub.heads 2 s with '/' :: '>' :: _ -> 2 | '>' :: _ -> 1 | _ -> raise Fail
  in
...
let parse s0 =
  ...
  | Some '-' -> (setext_heading ||| thematic_break ||| unordered_list_item ind) s
  | Some '_' -> thematic_break s
  | Some '#' -> atx_heading s
  | Some ('~' | '`') -> fenced_code ind s
  | Some '<' -> raw_html s
  | Some '*' -> (thematic_break ||| unordered_list_item ind) s
  | Some '+' -> (tag ind ||| unordered_list_item ind) s
...
let inline_attribute_string s =
  ...
  if attr = Attributes.empty then set_pos s ppos;
  attr
;;
...
let entity buf st =
  ...
  | Some '#' ->
    ...
    | Some ('x' | 'X') ->
      ...
      let rec aux n m =
        if n > 8
        then Buffer.add_string buf (range st p (pos st - p))
        else (
          match peek st with
          | Some ('0' .. '9' as c) ->
            junk st;
    ...
    | (Emph (pre, _, q1, n1) as x) :: xs when is_opener x ->
      let rec loop acc = function
        | (Emph (_, post, q2, n2) as x) :: xs when is_closer x && q1 = q2 ->
          let xs =
            if n1 >= 2 && n2 >= 2
            then if n2 > 2 then Emph (Punct, post, q2, n2 - 2) :: xs else xs
            else if n2 > 1
            then Emph (Punct, post, q2, n2 - 1) :: xs
            else xs
          in
          let r =
            ...
            R
              (Emph
                 { kind
                 ; style = q1
                 ; content = concat (List.map to_r (parse_emph (List.rev acc)))
                 })
            :: xs
          in
          let r =
            if n1 >= 2 && n2 >= 2
            then if n1 > 2 then Emph (pre, Punct, q1, n1 - 2) :: r else r
            else if n1 > 1
...
let list p st =
  let rec loop () = match protect p st with () -> loop () | exception Fail -> () in
  loop ()
;;
...
let eol st = match peek st with Some '\n' -> junk st | Some _ -> raise Fail | None -> ()
  ...
  let rec loop n =
    ...
    | '@' ->
      ...
      let label st =
        ...
        let rec loop last =
          match let_dig st with r -> loop r | exception Fail -> if last then raise Fail
        in
        loop false
      in
      label st;
...
let rec inline defs st =
  ...
  in
  let text acc =
    if Buffer.length buf = 0 then acc else Pre.R (Text (get_buf ())) :: acc
  in
  let rec reference_link kind acc st =
    ...
    | lab ->
      ...
      let reflink lab' =
        ...
        match
          List.find_opt (fun ({ label; _ } : string Link_def.t) -> label = s) defs
        with
        | Some def -> loop (Pre.R (Ref { kind; label = lab1; def }) :: text acc) st
        | None ->
          ...
          (Pre.R
             (Link
                ...
                ; def =
                    ...
                    ; attributes = inline_attribute_string st
                    }
                })
          :: text acc)
          st
              ...
              then
                loop
                  (Pre.R
                     (Code
                        { level = n
                        ; content = Buffer.contents bufcode
                        ; attributes = inline_attribute_string st
                        })
                  :: acc)
                  st
      ...
      let rec loop2 n =
        ...
        | Some _ ->
          ...
          let rec loop3 start seen_ws end_tag m bufcode =
            ...
            | Some '+' ->
              junk st;
              loop3 start seen_ws end_tag (succ m) bufcode
            | Some (' ' | '\t' | '\010' .. '\013') ->
              if m = n
              then
                loop
                  (Pre.R
                     (Tag
                        { tag = Buffer.contents tag
                        ; content = inline defs (Buffer.contents contents |> of_string)
                        ; attributes = inline_attribute_string st
                        })
                  :: acc)
                  st
                ...
                then (
                  ...
                  Buffer.add_string bufcode (String.make m '+'));
                junk st;
                loop3 (start && m = 0) end_tag true 0 contents)
            | Some c ->
              if m = n
              then
                loop
          ...
          | Some '[' ->
            ...
            | lab ->
              ...
              (match
                 List.find_opt (fun ({ label; _ } : string Link_def.t) -> label = s) defs
               with
              | Some def -> loop (Pre.R (Ref { kind = k; label; def }) :: acc') st
              | None ->
...
let link_reference_definition st =
  ...
  in
  let ws1 st =
    match next st with ' ' | '\t' | '\010' .. '\013' -> ws st | _ -> raise Fail
  in
  ignore (sp3 st);
  ...
  | title -> { Ast.Link_def.label; destination; title = Some title; attributes }
  | exception Fail ->
    (sp >>> eol) st;
