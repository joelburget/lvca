...
module Change = struct
  ...
  type t =
    ...
    ; to_ : position
    ; text : string array
    ; removed : string (* ; origin *)
    }

  (* type js = { } *)
end

(* class type keypress_event_type = object method end *)

class type codemirror_type =
  object
    method on_change : Js.js_string Js.t -> (Change.t -> unit) Js.callback -> unit Js.meth

    method on_keypress :
      Js.js_string Js.t
      -> (codemirror_type Js.t -> Dom_html.keyboardEvent Js.t -> unit) Js.callback
      -> unit Js.meth
...
let mk_codemirror
 ...
 fun initial_value ->
  ...
  let editor =
    Js.Unsafe.fun_call create_fn [| textarea |> To_dom.of_textarea |> Js.Unsafe.inject |]
  in
  wrapper, editor
;;
