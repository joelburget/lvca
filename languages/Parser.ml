    ...
    { run =
        (fun ~translate_direct:_ ~term_ctx ~parser_ctx:_ ~pos str ->
          let err_msg =
            mk_error
              (Printf.sprintf
                 {|expected: satisfy (%s -> {%s})|}
                 name
                 (Core.to_string core_term))
          in
    ...
    let rec go ~ambient_prec left =
      ...
      | Some token ->
        ...
        | Atom (IntAtom i, pos) ->
          let (_ : token) = Queue.dequeue_exn tokens in
          let i = Z.of_int i in
          return ~pos (Count (pos, left, Term (Primitive (pos, Integer i))))
        | _ -> return ~pos left)
    in
  ...
  and sequence ~tokens =
    ...
    let rec go () =
      ...
      | Some (Operator ("->", _)) ->
        ...
        (match Queue.dequeue tokens with
        | Some (Core (tm, pos)) -> return ~pos (Sequence (pos, Queue.to_list binders, tm))
        | Some (Ident (name, pos)) ->
          return
            ~pos
            (Sequence
