    ...
    ->
      begin
        ...
        match t1' with
          ...
          ->
            let%bind body' = match DeBruijn.open_scope t2' body with
              | [tm] -> Ok tm
              | tms ->
                let tms_str = tms
                  |> List.map ~f:tm_str
            ...
            eval' body'
          | _ -> Error
            (Printf.sprintf "Unexpected term (1) %s" (tm_str tm))
      end
    | Operator (_, "lam", _)
    -> Ok tm
...
module AngstromParse(Comment : ParseUtil.Comment_int) = struct
  ...
  let t_var : OptRange.t Nominal.term Parsers.t
    = Parsers.identifier >>||
      (fun ~pos name -> Nominal.Var (pos, name), pos)
  ...
  let rec pp' prec ppf tm =
    ...
    begin
      match tm with
        ...
        | tm ->
          Fmt.failwith "Invalid Lambda term %a" Nominal.pp_term tm
    end;
    OptRange.close_stag ppf (Nominal.location tm ); (* range tag *)
...
let%test_module "Lambda Calculus" = (module struct
  ...
  let pretty_eval_parse str = parse str
    |> Result.ok_or_failwith
    |> eval
    |> Result.ok_or_failwith
    |> Fmt.pr "%a" pp
