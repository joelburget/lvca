...
let to_string t = t
  |> Map.to_alist
  |> List.map
    ~f:(fun (buf, ranges) -> Caml.Printf.sprintf "%s:%s" buf (Ranges.to_string ranges))
  |> String.concat ~sep:","
...
let union x y = Map.merge x y ~f:(fun ~key:_ -> function
  | `Left v | `Right v -> Some v
  | `Both (v1, v2) -> Some (Ranges.union v1 v2)
  )
...
let of_opt_range ~buf = function
  | Some range -> of_range ~buf range
  | None -> empty

let is_subset p1 p2 = Map.for_alli p1 ~f:(fun ~key ~data:r1 -> match Map.find p2 key with
  | None -> false
  | Some r2 -> Ranges.is_subset r1 r2)
