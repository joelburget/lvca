...
module Make (Prim : LanguageObject_intf.S) = struct
  ...
  let rec to_plain = function
    ...
    | Primitive prim -> Plain.Primitive (Prim.to_plain prim)
  ...
  let rec of_plain = function
    ...
    | Primitive prim -> Primitive (Prim.of_plain prim)
    | Var name -> Var ((), name)
  ...
  let prim_pp ppf prim =
    Prim.pp_generic ~open_loc:(fun _ _ -> ()) ~close_loc:(fun _ _ -> ()) ppf prim
  ;;
    ...
    Util.Json.(
      ...
      | Operator (_, tag, tms) ->
        array
          ...
          |]
      | Primitive p -> array [| string "p"; prim_jsonify p |]
  ...
  module Properties = struct
    ...
    module Prim = Prim.Parse (ParseUtil.NoComment)
    open PropertyResult
