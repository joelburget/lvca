...
let of_opt_range = function
  | None -> []
  | Some rng -> [rng]

let to_string ranges = ranges
...
let rec fixup_ordered xs = match xs with
  | [] | [_] -> xs
  | x :: x' :: xs ->
...
let rec union r1 r2 =
  let go r1 r2 = match r1, r2 with
    ...
    | x :: xs, y :: ys ->
      ...
      then y :: union r1 ys
      else (* x and y overlap (and their union could also overlap) *)
        let xy = Range.union x y in
        union (xy :: xs) ys
  in go r1 r2 |> fixup_ordered

let rec is_subset r1 r2 = match r1, r2 with
  | [], _ -> true
  | _, [] -> false
  | x :: xs, y :: ys ->
...
let rec intersect r1 r2 = match r1, r2 with
  | [], _
  | _, [] -> []
  | x :: xs, y :: ys ->
    if x.Range.finish <= y.Range.start (* x doesn't overlap with any range *)
...
let%test_module "Ranges" = (module struct
  ...
  let%expect_test _ =
    ...
    [%expect{| |}] (* TODO: how to represent empty? *)
