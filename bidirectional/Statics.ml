...
and string_of_scope (Scope (pats, tms)) =
  let string_of_terms tms =
    tms |> List.map ~f:string_of_term |> String.concat ~sep:", "
  in
  match pats with
  | [] -> string_of_terms tms
  | _ ->
...
let erase_typing_rule { tm; ty } = { tm = erase_term tm; ty = erase_term ty }
...
let erase_hypothesis (env, clause) = Map.map env ~f:erase_term, erase_typing_clause clause
...
let erase_typing (Typing (t1, t2)) = Typing (erase_term t1, erase_term t2)
...
module Parse (Comment : ParseUtil.Comment_int) = struct
 ...
 ;;
  ...
  let term : Range.t term Parsers.t = cvt_tm <$> Term.t <?> "term"
  ...
  let t : Range.t rule list Parsers.t = many rule
...
(* open Binding
   ...
   let rec term_to_term : term -> Nominal.term = fun tm -> match tm with | Operator (name,
   subtms) (* We can't translate directly to [Operator (name, ...)] because we need to use
   [Operator] for free and bound vars. *) -> Operator (name, List.map subtms
   ~f:scope_to_term) | Bound _ -> Lvca_util.invariant_violation (Printf.sprintf
   "term_to_term: Bound variables are not allowed in typing rules: %s" (string_of_term tm)
   ) | Free name -> Var name | Primitive p -> Primitive p

   and scope_to_term : scope -> Nominal.scope = fun (Scope (pats, tm)) -> Scope (pats,
   term_to_term tm)

   let typing_rule_to_term : typing_rule -> Nominal.term = fun { tm; ty } -> Operator
   ("typing_rule", [ Scope ([], term_to_term tm) ; Scope ([], term_to_term ty) ])

   let typing_clause_to_term : typing_clause -> Nominal.term = function | InferenceRule
   rule -> Operator ("inference_rule", [Scope ([], typing_rule_to_term rule)]) |
   CheckingRule rule -> Operator ("checking_rule", [Scope ([], typing_rule_to_term rule)])

   let hypothesis_to_term : hypothesis -> Nominal.term = fun (ctx, typing_clause) ->
   Operator ("hypothesis", [ Scope ([], Sequence (ctx |> Map.to_alist |> List.map ~f:(fun
   (name, tm) -> Nominal.Operator ("pair", [ Scope ([], Primitive (PrimString name)) ;
   Scope ([], term_to_term tm) ])) )) ; Scope ([], typing_clause_to_term typing_clause) ])

   let term_of_option : ('a -> Nominal.term) -> 'a option -> Nominal.term = fun f ->
   function | None -> Operator ("none", []) | Some a -> Operator ("some", [Scope ([], f
   a)])

   let rule_to_term : rule -> Nominal.term = fun { hypotheses; name; conclusion } ->
   Operator ("rule", [ Scope ([], Sequence (List.map hypotheses ~f:hypothesis_to_term)) ;
   Scope ([], term_of_option (fun name' -> Primitive (PrimString name')) name) ; Scope
   ([], hypothesis_to_term conclusion) ])

   let to_term : rule list -> Nominal.term = fun rules -> Sequence (List.map rules
   ~f:rule_to_term) *)
