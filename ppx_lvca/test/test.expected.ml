...
module Lang(Integer:LanguageObject.AllTermS) :
  ...
  struct
    ...
    module Pair_plus(A:LanguageObject.AllTermS)(B:LanguageObject.AllTermS) =
      struct
        type 'info t = ('info, 'info A.t, 'info B.t) Wrapper.Types.pair_plus
        module Plain =
          struct type t = (A.Plain.t, B.Plain.t) Wrapper.Plain.pair_plus end
        let info tm = Wrapper.Info.pair_plus A.info B.info tm
        let to_plain tm = Wrapper.ToPlain.pair_plus A.to_plain B.to_plain tm
        let of_plain tm = Wrapper.OfPlain.pair_plus A.of_plain B.of_plain tm
        let equal ~info_eq  tm =
