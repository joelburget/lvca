  ...
  sig
    ...
    sig
      ...
      and 'info nat =
        ...
        | S of 'info * 'info nat 
      and 'info nonempty =
        | Nonempty of 'info * 'string * 'list 
      and ('info, 'a, 'b) pair =
        | Pair of 'info * 'a * 'b 
      and ('info, 'a, 'b) pair_plus =
        | PairPlus of 'info * 'a * 'b * 'info foo 
      and 'info term =
        | Operator of 'info * 'list 
    end
    module Plain :
    sig
      type foo =
        | Foo of 'integer 
        | Bar of (Pattern.Plain.t * Lvca_syntax.Single_var.Plain.t * foo) 
        | Foo_var of string 
      and ifz =
    ...
    module Pair :
    Lvca_syntax.Nominal.Convertible.Extended_s with type ('info,'a,'b) t = 
      ('info, 'info A.t, 'info B.t) Types.pair and type ('a,'b) Plain.t = 
      (A.Plain.t, B.Plain.t) Plain.pair
    module Pair_plus :
    Lvca_syntax.Nominal.Convertible.Extended_s with type ('info,'a,'b) t = 
      ('info, 'info A.t, 'info B.t) Types.pair_plus and type
      ('a,'b) Plain.t =  (A.Plain.t, B.Plain.t) Plain.pair_plus
    module Nonempty :
    Lvca_syntax.Nominal.Convertible.Extended_s with type 'info t = 
          ...
          struct
            ...
            let nonempty =
              ...
              | Plain.Nonempty (x1, x2) ->
                  Types.Nonempty ((), (string x1), (list x2))
            let pair f_a f_b =
              function
            ...
            let term =
              function | Plain.Operator x1 -> Types.Operator ((), (list x1))
            let rec ifz =
              function
          ...
          struct
            ...
            let nonempty ~f  =
              ...
              | Types.Nonempty (x0, x1, x2) ->
                  Types.Nonempty ((f x0), (string ~f x1), (list ~f x2))
            let pair f_a f_b ~f  =
              function
              | Types.Pair (x0, x1, x2) ->
          ...
          struct
            ...
            let term =
              ...
              | Types.Operator (x0, x1) ->
                  Lvca_syntax.Nominal.Term.Operator
                    (x0, "Operator",
                      [Lvca_syntax.Nominal.Scope.Scope ([], (list x1))])
            let rec ifz =
              function
              | Types.Ifz (x0, x1, (x2, x3), x4) ->
                  Lvca_syntax.Nominal.Term.Operator
