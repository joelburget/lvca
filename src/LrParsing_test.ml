...
(* I1 *)
let items1 = [ mk_item' 0 1; mk_item' 1 1 ]
...
let%test_module "lr0_items" =
  (module struct
    let expected_lr0_item_sets = Util.MutableSet.of_list (module IntSet) lr0_item_sets
...
let%test_module "lr0 goto table" =
  (module struct
    let ( = ) = Caml.( = )
...
let%test_module "lr0 action table" =
  (module struct
    (* Test for a match with CPTT Figure 4.37 *)
