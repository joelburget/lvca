...
let check_assert : bool -> unit
  = function
    | true -> ()
    | false -> raise CheckFailure

let to_string : DeBruijn.term -> string
  = fun tm -> match DeBruijn.to_nominal tm with
...
let rec ty_to_string : ty -> string
  = function
    ...
    | Arr (t1, t2) -> Printf.sprintf
      "arr(%s; %s)"
      (ty_to_string t1)
      (ty_to_string t2)

let rec ty_of : DeBruijn.term -> ty
  = function
    | Operator ("bool", []) -> Bool
    | Operator ("arr", [Scope([], t1); Scope([], t2)])
...
let rec check : ty list -> ty -> DeBruijn.term -> bool
  ...
  -> (match infer env f with
    | Some (Arr (t1, t2)) ->
      check (t1 :: env) t2 f && check env t1 arg
    | _ -> false
  )
...
and infer : ty list -> DeBruijn.term -> ty option
  ...
  -> check_assert (check env Bool cond);
     (match infer env b1 with
       | Some t1 -> check_assert (check env t1 b2); Some t1
       | None -> (match infer env b2 with
         | Some t2 -> check_assert (check env t2 b1); Some t2
         | None -> None
       )
     )
...
let check_eval : Nominal.term -> ((string * string), string) Result.t
  = fun tm -> match DeBruijn.from_nominal tm with
    | Error err -> Error err
    | Ok tm' -> match infer [] tm' with
      | Some ty -> Ok (ty_to_string ty, to_string (eval' [] tm'))
      | None -> Error "couldn't infer type"
      | exception (Unknown tm)
      -> Error ("Unknown operator: " ^ to_string tm)
