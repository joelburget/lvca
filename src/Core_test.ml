...
(*
let true_tm   = DeBruijn.Operator ("true", [])
let false_tm  = DeBruijn.Operator ("false", [])
*)
...
let%test_module "Core" = (module struct
  ...
  let%test "to_ast 2" = to_ast (Operator
        ( "foo"
  ...
  let%test "eval 5" = eval (binary_int_op "#add" 1 2)
    = Ok (Primitive (PrimInteger (Bigint.of_int 3)))
  let%test "eval 6" = eval (binary_int_op "#sub" 1 2)
    = Ok (Primitive (PrimInteger (Bigint.of_int (-1))))
end)
