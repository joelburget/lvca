...
let rec pattern_to_tree : sort_name -> Pattern.t -> tree =
  ...
  | Primitive p ->
    ...
    in
    mk_tree sort_name (Primitive prim_ty) [| mk_terminal_capture str |]
;;
...
let rec of_ast
  ...
  | _, Var name -> mk_tree sort_name SingleCapture [| mk_terminal_capture name |]
...
and go_operator = fun
  ...
  *)
  ...
  |> mk_tree sort_name (Operator op_name)
;;
