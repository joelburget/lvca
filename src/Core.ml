...
let rec from_ast' (vars : S.t) (term : Nominal.term) : core =
  match term with
...
let from_ast : Nominal.term -> core = from_ast' S.empty
...
let rec to_ast (core : core) : Nominal.term =
  match core with
  | Var name -> Var name
  | Operator (tag, vals) -> Operator (tag, List.map vals scope_to_ast)
  | Primitive prim -> Primitive prim
  | Sequence tms -> Sequence (List.map tms to_ast)
...
let rec match_branch v pat =
  ...
  | Primitive l1, Primitive l2 -> if prim_eq l1 l2 then Some M.empty else None
  | _, Var "_" -> Some M.empty
  | tm, Var v -> Some (M.fromArray [| v, tm |])
  | _ -> None
;;

let rec find_core_match v : core_scope list -> (core * core M.t) option = function
  | [] -> None
...
let rec fill_in_core ({ dynamics; vars; assignments } as args) = function
  ...
  | CoreApp (f, app_args) ->
    CoreApp (fill_in_core args f, app_args |. List.map (fill_in_core args))
