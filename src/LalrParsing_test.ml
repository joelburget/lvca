...
module Grammar3 : GRAMMAR = struct
  let grammar =
    AugmentedGrammar
      { nonterminals =
          [| "arith'", 0, { productions = [ [ Nonterminal 1 ] ] }
           ; (* arith' -> arith *)
             ( "arith"
             , 1
             , { productions =
...
type lookahead_item_sets = (LookaheadItemSet.t, LookaheadItemSet.comparator_witness) Set.t
...
let lookahead_item_set_set : LookaheadItemSet.t list -> lookahead_item_sets =
...
let mk_item_set pruduction_num position = SI.of_list [ mk_item' pruduction_num position ]
...
let mk_set : item_set list -> item_set_set = Set.of_list (module LrParsing.IntSet)
...
let%test "grammar 2 lalr1 kernel items" =
  Set.equal
    actual_grammar2_lalr1_kernels
    (expected_grammar2_lalr1_kernels |> List.map ~f:(fun (_, k) -> k) |> mk_set)
;;
...
let equivalent_propagation : (state * item) array -> (state * item) array -> bool =
 fun p1 p2 ->
  Set.equal (Set.of_array (module Propagation) p1) (Set.of_array (module Propagation) p2)
;;
...
let equivalent_generation
  ...
  =
 fun g1 g2 ->
  Set.equal (Set.of_array (module Generation) g1) (Set.of_array (module Generation) g2)
;;
...
let mk_wrapper prod_num ({ start_pos; end_pos; _ } as child) =
  { production = Either.Second prod_num; children = [ child ]; start_pos; end_pos }
;;

let state =
  gram1_lr1_config_sets
  |> Array.map ~f:(fun { LalrParsing.kernel_items; _ } ->
         Grammar1LR.item_set_to_state @@ lookahead_item_set_to_item_set kernel_items)
;;

let ( = ) = Caml.( = )

let expect_no_goto start_state nt_num =
  Grammar1Lalr.lalr1_goto_table state.(start_state) (Nonterminal nt_num) = None
;;

let expect_goto start_state nt_num result_state =
...
let%test_module "lalr1_action_table" =
  (module struct
    ...
    let action_table_tests' () =
      List.iter action_table_tests ~f:(fun (init_state, terminal_num) ->
          Printf.printf
            "%n %s -> %s\n"
            init_state
...
let%test_module "parse" =
  (module struct
    let%test "cdd" =
       ...
       *)
      let tokens1 =
        Queue.of_list [ mk_tok "c" 0 1; mk_tok "d" 1 2; mk_tok "d" 2 3; mk_tok "$" 3 3 ]
      in
      Grammar1Lalr.parse (* "cdd" *) tokens1
      = Ok
          { production = Either.Second 1
          ; children =
              [ { production = Either.Second 2
                ; children =
                    [ mk_terminal c_num 0 1; mk_wrapper 3 @@ mk_terminal d_num 1 2 ]
                ; start_pos = 0
                ; end_pos = 2
                }
                         ...
                         ; children =
                             ...
                             ; mk_wrapper 5
                               @@ { production = Either.Second 4
                                  ; children = [ mk_terminal id_num 2 5 ]
                                  ; start_pos = 2
                                  ; end_pos = 5
                                  }
                             ]
                ...
                @@ { production = Either.Second 3
                   ; children =
                       ...
                       ; mk_wrapper
                           5
                           { production = Either.Second 4
                           ; children = [ mk_terminal id_num 9 12 ]
                           ; start_pos = 9
                           ; end_pos = 12
                           }
                       ]
