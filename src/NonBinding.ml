...
let rec from_de_bruijn' = function
  ...
  | Sequence tms
  -> Sequence (tms |. BL.map from_de_bruijn')
  | Primitive p
  -> Primitive p
...
let from_de_bruijn (tm : DeBruijn.term): term option
  = try
      Some (from_de_bruijn' tm)
    with
      ScopeEncountered -> None

let rec to_de_bruijn tm : DeBruijn.term
  = match tm with
    | Operator (tag, tms) -> DeBruijn.Operator (tag,
      tms |. BL.map (fun tm -> DeBruijn.Scope ([], to_de_bruijn tm))
    )
...
let rec from_nominal' = function
  | Nominal.Operator (tag, scopes)
    -> Operator (tag, scopes |. BL.map from_nominal_scope)
  | Var _
  -> raise ScopeEncountered
  | Sequence tms
  -> Sequence (tms |. BL.map from_nominal')
  | Primitive p
  -> Primitive p
...
let from_nominal (tm : Nominal.term): term option
  = try
      Some (from_nominal' tm)
    with
      ScopeEncountered -> None

let rec to_nominal tm : Nominal.term
  = match tm with
    | Operator (tag, tms) -> Nominal.Operator (tag,
      tms |. BL.map (fun tm -> Nominal.Scope ([], to_nominal tm))
    )
