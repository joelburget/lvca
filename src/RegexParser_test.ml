...
let%test_module "Regex_Parser" =
  (module struct
    ...
    let%test_unit "(foo)(bar)" =
      expect_parse_and_rt "(foo)(bar)" (ReConcat [ ReString "foo"; ReString "bar" ])
    ;;
    ...
    let%test_unit "" = expect_parse_and_rt "(foo)?" (ReOption (ReString "foo"))

    let%test_unit "foo|bar" =
      expect_parse_and_rt "foo|bar" (ReChoice [ ReString "foo"; ReString "bar" ])
    ;;
