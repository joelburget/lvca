...
let%test_module "Regex_Parser" = (module struct
  ...
  let%test_unit "foo" = expect_parse_and_rt
    "foo"
    (ReString "foo")
  let%test_unit "(foo)(bar)" = expect_parse_and_rt
    "(foo)(bar)"
    (ReConcat [ReString "foo"; ReString "bar"])
  let%test_unit "[a-z]" = expect_parse_and_rt
    "[a-z]"
    lower_alpha
  let%test_unit "[a-zA-Z]" = expect_parse_and_rt
    "[a-zA-Z]"
    alpha
  let%test_unit "(foo)*" = expect_parse_and_rt
    "(foo)*"
  ...
  let%test_unit "(foo)+" = expect_parse_and_rt
    "(foo)+"
    (RePlus (ReString "foo"))
  let%test_unit "" = expect_parse_and_rt
    "(foo)?"
  ...
  let%test_unit "[a-z][a-zA-Z0-9]*" = expect_parse_and_rt
    "[a-z][a-zA-Z0-9]*"
    (ReConcat [lower_alpha; ReStar words])
  let%test_unit "([a-z][a-zA-Z0-9])*" = expect_parse_and_rt
    "([a-z][a-zA-Z0-9])*"
    (ReStar (ReConcat [lower_alpha; words]))

  let%test_unit "[ ]+" = expect_parse_and_rt
    "[ ]+"
    (RePlus (ReSet [SingleCharacter ' ']))
  let%test_unit " +" = expect_parse_and_rt
    " +"
    (RePlus (ReString " "))
  ...
  let%test_unit {|\\|} = expect_parse_and_rt
    {|\\|}
    (ReString {|\|})
  let%test_unit {|\b|} = expect_parse_and_rt
    {|\b|}
