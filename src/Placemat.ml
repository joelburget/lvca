...
module List = struct
  ...
  let mapi : f:(int -> 'a -> 'b) -> 'a list -> 'b list
...
module MutableMap = struct
  ...
  module Impl = struct
    ...
    let add : ('key, 'value, 'id) t -> key:'key -> value:'value -> unit
      = fun ({ cmp; data; size } as map) ~key ~data ->
        let added = ref false in
        let new_table =
...
module MutableSet = struct
  ...
  module Impl = struct
    ...
    let has : ('value, 'a) t -> value:'value -> bool
      = fun t ~data -> MutableMap.has t ~key:value

    let add : ('value, 'id) t -> value:'value -> unit
      = fun set ~data -> MutableMap.add set ~key:value ~data:()
...
module Core_kernel.Stack = struct
...
module Core_kernel.Queue = struct
...
module Int.Map = struct
  ...
  let values_to_array dict = dict
    |> to_array
    |> Base.Array.map ~f:(fun (_, v) -> v)
end

module String.Map = struct
...
module Lex : sig
  ...
  val lex : lexer -> string -> (token array, lex_error) Base.Result.t
end = struct
  type token =
    { name : string
    ; start : int (* inclusive *)
  ...
  let string_of_tokens : token array -> string
    = fun toks -> toks
                  |> Base.Array.map ~f:(fun { name; _ } -> name)
                  |> Base.String.concat_array ~sep:" "

  (** raises: [LexError] *)
  let get_next_tok_exn : string Core_kernel.Int.Map.t -> Str.regexp -> lexbuf -> token
    = fun tok_names re { buf; pos } ->

      let input = Core_kernel.String.slice buf
        pos
        (Base.String.length buf)
      in
  ...
  let tokenize : lexer -> string -> token array
    = fun lexer input ->
      ...
      let mut_tok_names = Core_kernel.Int.Table.create () in

      let re_str = lexer
        |> List.mapi (fun i (re_str, tok_name) ->
          Core_kernel.Int.Table.set mut_tok_names ~key:i ~data:tok_name;
          "(" ^ re_str ^ ")")
      ...
      (*
      mut_tok_names
        |> Core_kernel.Int.Table.to_array
        |> Int.Map.from_array
        *)
      in
      let re = Str.regexp re_str in

      while lexbuf.pos < Base.String.length lexbuf.buf do
        let tok = get_next_tok_exn tok_names re lexbuf in
        let { start; finish; _ } = tok in
        assert (start = lexbuf.pos);
        lexbuf.pos <- finish;
        ignore (Core_kernel.Queue.enqueue result tok : unit)
      done;

      Core_kernel.Queue.to_array result

  let lex : lexer -> string -> (token array, lex_error) Base.Result.t
    = fun lexer input ->
      try
